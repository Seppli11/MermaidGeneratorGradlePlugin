...
public class SiedlerGameTestBasic {
    ...
    public void requirementLandPlacementTest() {
        ...
        for (Map.Entry<Point, Config.Land> e : Config.getStandardLandPlacement().entrySet()) {
            ...
            assertEquals(e.getValue(), model.getBoard().getField(e.getKey()).getFieldType(),
    ...
    public void requirementDiceThrowResourcePayoutThreePlayerStandardTest(int diceValue) {
        SiedlerGame model = ThreePlayerStandard.getAfterSetupPhase(DEFAULT_WINPOINTS);
        Map<Config.Faction, List<Config.Resource>> expectd = ThreePlayerStandard.INITIAL_DICE_THROW_PAYOUT
                .get(diceValue);
    ...
    public void requirementDiceThrowPlayerResourceCardStockUpdateTest() {
        ...
        Map<Config.Faction, Map<Config.Resource, Integer>> expected = Map.of(
                ...
                Map.of(Config.Resource.GRAIN, 1, Config.Resource.WOOL, 5, Config.Resource.BRICK, 0, Config.Resource.ORE,
                        0, Config.Resource.LUMBER, 0),
                Config.Faction.values()[2], Map.of(Config.Resource.GRAIN, 0, Config.Resource.WOOL, 0,
                        Config.Resource.BRICK, 2, Config.Resource.ORE, 0, Config.Resource.LUMBER, 1));
    ...
    public void requirementTwoSettlementsSamePlayerSameFieldResourceCardPayout() {
        ...
        assertEquals(List.of(Config.Resource.ORE, Config.Resource.ORE),
                model.throwDice(4).get(model.getCurrentPlayerFaction()));
    }
    ...
    public void requirementCanTradeFourToOneWithBank() {
        ...
        assertEquals(expectedResourceCards.get(Config.Resource.WOOL),
                model.getCurrentPlayerResourceStock(Config.Resource.WOOL));
        assertEquals(expectedResourceCards.get(Config.Resource.LUMBER),
        ...
        int cardsOffered = 4;
        int cardsReceived = 1;
        assertEquals(expectedResourceCards.get(Config.Resource.WOOL) - cardsOffered,
                model.getCurrentPlayerResourceStock(Config.Resource.WOOL));
        assertEquals(expectedResourceCards.get(Config.Resource.LUMBER) + cardsReceived,
                model.getCurrentPlayerResourceStock(Config.Resource.LUMBER));
    }
